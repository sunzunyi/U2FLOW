...
from ..ar_transforms.interpolation import Interp2
from ..ar_transforms.interpolation import Meshgrid
import numpy as np
...
class _IdentityParams(nn.Module):
    ...
    def _update(self, batch_size):
        ...
        return torch.cat([self._i, self._o, self._o, self._o, self._i, self._o], dim=1) # batch_size x 6
...
class RandomAffineFlow(nn.Module):
    ...
    def apply_random_transforms_to_params(self,
                                          ...
                                          validate_size=None):
        max_translate *= 0.5
        batch_size = theta0.size(0) # batch_size x 6
        ...
        while invalid.sum() > 0:
            ...
            thetas = invalid.float() * theta_try + torch.logical_not(invalid).float() * thetas
    ...
    def forward(self, data):
        ...
        uncer = [
            self.transform_image(unc, theta)
            for unc, theta in zip(uncer, theta_list)
        ]
