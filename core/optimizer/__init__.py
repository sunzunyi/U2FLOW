...
def other_parameters(module):
    return [param for name, param in module.named_parameters() if '.bias' not in name and '.weight' not in name]
...
def build_optimizer(model, config):
    ...
    if name == "adam":
        ...
        param_groups = [
            {'params': bias_parameters(model.module),
             'weight_decay': config.bias_decay},
            {'params': weight_parameters(model.module),
             'weight_decay': config.weight_decay},
            {'params': other_parameters(model.module),
             'weight_decay': 0} ]
...
def build_scheduler(config, optimizer):
    ...
    if name == 'OneCycleLR':
        ...
        if hasattr(config, 'twins_lr_factor'):
            ...
            # params = config.lr_scheduler.params
            # params['epochs'] = min(epoches, self.cfg.epoch_num - self.i_epoch)
            # params['steps_per_epoch'] = self.cfg.epoch_size
            scheduler = OneCycleLR(optimizer, max_lr=config.max_lr, total_steps=int(config.num_steps+100),
                pct_start=config.pct_start, cycle_momentum=config.cycle_momentum, anneal_strategy=config.anneal_strategy)
