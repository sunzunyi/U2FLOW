...
import core.flow_datasets_sintel as datasets
...
def train(cfg):
    ...
    while should_keep_training:
        ...
        if cfg.is_main_process and tqdm_flag:
        ...
        for i_batch, data_blob in enumerate(train_loader):
            ...
            if logger.total_steps % cfg.val_freq == 0 and cfg.is_main_process:
                ...
                for val_dataset in cfg.validation:
                    if val_dataset == 'MpiSintel': # sintel_train
                        results.update(evaluate.validate_sintel_batch(model.module, logger))
                epeval = 1e10
                for key in results:
                    if 'epe' in key:
                        epeval = results[key]
